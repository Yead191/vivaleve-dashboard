...
import { sendHistory, BroadcastHistory } from '../../data/mockData';
import { ColumnsType } from 'antd/es/table';

export default function SendHistoryPanel() {
  const { message, modal } = App.useApp();
  const [history, setHistory] = useState<BroadcastHistory[]>(sendHistory);
  const [detail, setDetail]   = useState<BroadcastHistory | null>(null);

  const cols: ColumnsType<BroadcastHistory> = [
