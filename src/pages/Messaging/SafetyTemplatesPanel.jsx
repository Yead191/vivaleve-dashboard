...
import { safetyTemplates } from '../../data/mockData.js';

export default function SafetyTemplatesPanel() {
  const { message, modal } = App.useApp();
  const [tpls, setTpls]   = useState(safetyTemplates);
  const [editing, setEditing] = useState(null);
  const [open, setOpen] = useState(false);
  const [form] = Form.useForm();

  const openAdd  = () => { setEditing(null); form.resetFields(); setOpen(true); };
  const openEdit = (t) => { setEditing(t); form.setFieldsValue(t); setOpen(true); };
