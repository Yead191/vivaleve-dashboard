  ...
  const handleAction = (key, user) => {
    if (key === 'suspend')      setSuspendUser(user);
      ...
      modal.confirm({
        ...
        okText: 'Activate',
        onOk: () => message.success(`${user.name} activated`),
      });
    } else if (key === 'impersonate') {
      message.info(`Opening view-as session for ${user.name}…`);
    }
  };
    ...
    <div className="space-y-6">
      ...
      {tab === 'all' && (
        <>
          <UsersFilterBar
            ...
            onExport={() => message.success('Exporting to CSV…')}
          />
      ...
      <SuspendUserModal
        ...
        onConfirm={({ user, duration, reason }) => {
          message.success(`${user.name} suspended for ${duration}`);
          setSuspendUser(null);
        }}
      />
      <BanUserModal
