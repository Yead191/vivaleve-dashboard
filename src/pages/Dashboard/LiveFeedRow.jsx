...
import { recentSignups, recentReports, recentPurchases } from '../../data/mockData.js';
...
export default function LiveFeedRow() {
  ...
  const reportCols = [
    { title: 'Target', dataIndex: 'target', key: 'target', render: v => <span className="text-[13px] font-medium text-gray-900">{v}</span> },
    { title: 'Reason', dataIndex: 'reason', key: 'reason', render: v => <span className="text-[12px] text-gray-600">{v}</span> },
    { title: 'When',   dataIndex: 'when',   key: 'when',   width: 90, render: v => <span className="text-[12px] text-gray-500">{v}</span> },
  ...
  const purchaseCols = [
    ...
    { title: 'Plan',   dataIndex: 'plan',   key: 'plan',   render: v => <span className="text-[12px] text-gray-600">{v}</span> },
    { title: 'Amount', dataIndex: 'amount', key: 'amount', width: 90, render: v => <span className="text-[12px] font-medium text-gray-900">{v}</span> },
