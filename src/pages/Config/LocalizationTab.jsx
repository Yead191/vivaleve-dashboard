...
import { localizationStrings, locales as initLocales } from '../../data/mockData.js';
...
export default function LocalizationTab() {
  ...
  const [strings, setStrings]   = useState(localizationStrings);
  const [locales, setLocales]   = useState(initLocales);
  const [addStringOpen, setAddStringOpen] = useState(false);
  const [addLocaleOpen, setAddLocaleOpen] = useState(false);
  const [stringForm] = Form.useForm();
  const [localeForm] = Form.useForm();

  const updateCell = (id, locale, value) => {
