...
import { matchFunnel, peakHours } from "../../data/mockData";
...
export default function MatchEngagementSection() {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-[12px] font-medium uppercase tracking-wider text-gray-400 px-1">
        Match & engagement
      </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <SectionCard
          ...
          description="Views → Likes → Matches → Messages"
                    ...
                    <span className="text-[12px] text-gray-500">
                      ...
                      {i > 0 && (
                        <span className="ml-2 text-[11px] text-rose-500">
                          (
                          {((s.value / matchFunnel[i - 1].value) * 100).toFixed(
                            1,
                          )}
                          %)
                        </span>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-card p-5">
      ...
      <div className="text-[24px] font-semibold text-gray-900 mt-1">
        {value}
      </div>
    </div>
  );
