...
import { moderationRules } from '../../data/mockData.js';

export default function AutoModerationTab() {
  const { message, modal } = App.useApp();
  const [rules, setRules]    = useState(moderationRules);
  const [editing, setEditing] = useState(null);
  const [openForm, setOpenForm] = useState(false);
  const [form] = Form.useForm();

  const openAdd = () => { setEditing(null); form.resetFields(); setOpenForm(true); };
  const openEdit = r => { setEditing(r); form.setFieldsValue(r); setOpenForm(true); };
  ...
  const cols = [
    ...
    { title: 'Action',dataIndex: 'action',key: 'action',width: 170,
      render: v => {
        const colors = { 'auto-remove': 'red', 'flag for review': 'orange', 'warn user': 'gold' };
