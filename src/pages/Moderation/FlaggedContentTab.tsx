  ...
  return (
    <div className="space-y-4">
        ...
        {filtered.map(item => {
          ...
          return (
            <div key={item.id} className="bg-white rounded-xl border border-gray-200 shadow-card p-5">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Button size="small" icon={<Check className="w-3.5 h-3.5" />} onClick={() => toast.success(`Approved · ${item.id}`)}>Approve</Button>
                <Button size="small" danger icon={<X className="w-3.5 h-3.5" />} onClick={() => toast.success(`Removed · ${item.id}`)}>Remove</Button>
      ...
      <Modal
        ...
        okText="Escalate"
        onOk={() => escForm.validateFields().then(() => { setEscalate(null); toast.success('Escalated to senior trust & safety'); escForm.resetFields(); })}
